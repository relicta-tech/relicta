...
func (s *ServiceImpl) CreateTag(_ context.Context, name, message string, opts TagOptions) error {
	...
	if opts.Annotated {
		// Determine tagger name and email
		taggerName := opts.TaggerName
		taggerEmail := opts.TaggerEmail

		// Fall back to git config values if not provided
		if taggerName == "" || taggerEmail == "" {
			cfg, cfgErr := s.repo.Config()
			if cfgErr == nil && cfg.User.Name != "" && taggerName == "" {
				taggerName = cfg.User.Name
			}
			if cfgErr == nil && cfg.User.Email != "" && taggerEmail == "" {
