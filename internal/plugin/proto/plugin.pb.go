...
type PluginInfo struct {
	...
	// safety declares the plugin's runtime requirements (ADR-008).
	// Absent on legacy plugins; the host treats that as undeclared.
	Safety        *SafetyRequirements `protobuf:"bytes,7,opt,name=safety,proto3" json:"safety,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *PluginInfo) GetSafety() *SafetyRequirements {
	if x != nil {
		return x.Safety
	}
	return nil
...
// SafetyRequirements declares what a plugin needs at runtime (ADR-008).
// The host validates the declaration against the active safety policy
// BEFORE registering the plugin and derives sandbox configuration from it.
// Plugins that declare nothing are treated as risk_class "runtime" —
// loadable only with explicit operator trust.
type SafetyRequirements struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// risk_class is one of: passive | active | runtime.
	//
	//	passive  — read-only observation (notifications, reporting)
	//	active   — mutates files, tags, or remote state (publishing)
	//	runtime  — arbitrary execution beyond declared scopes
	RiskClass string `protobuf:"bytes,1,opt,name=risk_class,json=riskClass,proto3" json:"risk_class,omitempty"`
	// network_hosts lists host globs the plugin connects to (e.g. "*.github.com").
	NetworkHosts []string `protobuf:"bytes,2,rep,name=network_hosts,json=networkHosts,proto3" json:"network_hosts,omitempty"`
	// file_paths lists path globs the plugin reads/writes.
	FilePaths []string `protobuf:"bytes,3,rep,name=file_paths,json=filePaths,proto3" json:"file_paths,omitempty"`
	// env_vars lists environment variables the plugin needs.
	EnvVars []string `protobuf:"bytes,4,rep,name=env_vars,json=envVars,proto3" json:"env_vars,omitempty"`
	// needs_confirmation requests an interactive confirmation before execution.
	NeedsConfirmation bool `protobuf:"varint,5,opt,name=needs_confirmation,json=needsConfirmation,proto3" json:"needs_confirmation,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}
...
const file_plugin_proto_rawDesc = "" +
	...
	"\rconfig_schema\x18\x06 \x01(\tR\fconfigSchema\x123\n" +
	"\x06safety\x18\a \x01(\v2\x1b.relicta.SafetyRequirementsR\x06safety\"\x97\x01\n" +
	...
	"\x04code\x18\x03 \x01(\tR\x04code\"\xc1\x01\n" +
	"\x12SafetyRequirements\x12\x1d\n" +
	"\n" +
	"risk_class\x18\x01 \x01(\tR\triskClass\x12#\n" +
	"\rnetwork_hosts\x18\x02 \x03(\tR\fnetworkHosts\x12\x1d\n" +
	"\n" +
	"file_paths\x18\x03 \x03(\tR\tfilePaths\x12\x19\n" +
	"\benv_vars\x18\x04 \x03(\tR\aenvVars\x12-\n" +
	"\x12needs_confirmation\x18\x05 \x01(\bR\x11needsConfirmation*\xc2\x02\n" +
	"\x04Hook\x12\x14\n" +
	...
	"\bValidate\x12\x18.relicta.ValidateRequest\x1a\x19.relicta.ValidateResponseB<H\x03Z8github.com/relicta-tech/relicta/v4/internal/plugin/protob\x06proto3"
...
var file_plugin_proto_goTypes = []any{
	...
	(*SafetyRequirements)(nil), // 12: relicta.SafetyRequirements
	nil,                        // 13: relicta.ReleaseContext.EnvironmentEntry
}
var file_plugin_proto_depIdxs = []int32{
	12, // 0: relicta.PluginInfo.safety:type_name -> relicta.SafetyRequirements
	0,  // 1: relicta.ExecuteRequest.hook:type_name -> relicta.Hook
	5,  // 2: relicta.ExecuteRequest.context:type_name -> relicta.ReleaseContext
	8,  // 3: relicta.ExecuteResponse.artifacts:type_name -> relicta.Artifact
	6,  // 4: relicta.ReleaseContext.changes:type_name -> relicta.CategorizedChanges
	13, // 5: relicta.ReleaseContext.environment:type_name -> relicta.ReleaseContext.EnvironmentEntry
	7,  // 6: relicta.CategorizedChanges.features:type_name -> relicta.ConventionalCommit
	7,  // 7: relicta.CategorizedChanges.fixes:type_name -> relicta.ConventionalCommit
	7,  // 8: relicta.CategorizedChanges.breaking:type_name -> relicta.ConventionalCommit
	7,  // 9: relicta.CategorizedChanges.performance:type_name -> relicta.ConventionalCommit
	7,  // 10: relicta.CategorizedChanges.refactor:type_name -> relicta.ConventionalCommit
	7,  // 11: relicta.CategorizedChanges.docs:type_name -> relicta.ConventionalCommit
	7,  // 12: relicta.CategorizedChanges.other:type_name -> relicta.ConventionalCommit
	11, // 13: relicta.ValidateResponse.errors:type_name -> relicta.ValidationError
	1,  // 14: relicta.Plugin.GetInfo:input_type -> relicta.Empty
	3,  // 15: relicta.Plugin.Execute:input_type -> relicta.ExecuteRequest
	9,  // 16: relicta.Plugin.Validate:input_type -> relicta.ValidateRequest
	2,  // 17: relicta.Plugin.GetInfo:output_type -> relicta.PluginInfo
	4,  // 18: relicta.Plugin.Execute:output_type -> relicta.ExecuteResponse
	10, // 19: relicta.Plugin.Validate:output_type -> relicta.ValidateResponse
