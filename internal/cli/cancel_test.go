...
import (
	...
	domainrelease "github.com/relicta-tech/relicta/v4/internal/domain/release/domain"
...
func TestValidateCancelState_PublishedState(t *testing.T) {
	rel := release.NewReleaseRunForTest("test-pub", "main", "/test/repo")
	rel.ReconstructState(domainrelease.RunSnapshot{
		ID:       "test-pub",
		State:    domainrelease.StatePublished,
		RepoRoot: "/test/repo",
	})

	err := validateCancelState(rel)
	if err == nil {
		t.Error("validateCancelState() should return error for published state")
	}
}

func TestValidateCancelState_PublishingState(t *testing.T) {
	rel := release.NewReleaseRunForTest("test-pub", "main", "/test/repo")
	rel.ReconstructState(domainrelease.RunSnapshot{
		ID:       "test-pub",
		State:    domainrelease.StatePublishing,
		RepoRoot: "/test/repo",
	})

	origForce := cancelForce
	cancelForce = false
	defer func() { cancelForce = origForce }()

	err := validateCancelState(rel)
	if err == nil {
		t.Error("validateCancelState() should return error for publishing state without force")
	}
}

func TestValidateCancelState_PublishingStateWithForce(t *testing.T) {
	rel := release.NewReleaseRunForTest("test-pub", "main", "/test/repo")
	rel.ReconstructState(domainrelease.RunSnapshot{
		ID:       "test-pub",
		State:    domainrelease.StatePublishing,
	...
	err := validateCancelState(rel)
	if err != nil {
		t.Errorf("validateCancelState() with force should allow publishing state, got: %v", err)
	}
}

func TestValidateCancelState_FailedStateNew(t *testing.T) {
	rel := release.NewReleaseRunForTest("test-fail", "main", "/test/repo")
	rel.ReconstructState(domainrelease.RunSnapshot{
		ID:       "test-fail",
		State:    domainrelease.StateFailed,
		RepoRoot: "/test/repo",
	})

	err := validateCancelState(rel)
	if err == nil {
		t.Error("validateCancelState() should return error for failed state")
	}
}

func TestValidateResetState_PublishedState(t *testing.T) {
	rel := release.NewReleaseRunForTest("test-pub", "main", "/test/repo")
	// Reconstruct with published state
	rel.ReconstructState(domainrelease.RunSnapshot{
		ID:       "test-pub",
		State:    domainrelease.StatePublished,
		RepoRoot: "/test/repo",
	...
	err := validateResetState(rel)
	if err == nil {
		t.Error("validateResetState() should return error for published state")
	}
}

func TestValidateResetState_PublishingState(t *testing.T) {
	rel := release.NewReleaseRunForTest("test-pub", "main", "/test/repo")
	rel.ReconstructState(domainrelease.RunSnapshot{
		ID:       "test-pub",
		State:    domainrelease.StatePublishing,
	...
	err := validateResetState(rel)
	if err == nil {
		t.Error("validateResetState() should return error for publishing state without force")
	}
}

func TestValidateResetState_PublishingStateWithForce(t *testing.T) {
	rel := release.NewReleaseRunForTest("test-pub", "main", "/test/repo")
	rel.ReconstructState(domainrelease.RunSnapshot{
		ID:       "test-pub",
		State:    domainrelease.StatePublishing,
	...
	err := validateResetState(rel)
	if err != nil {
		t.Errorf("validateResetState() with force should allow publishing state, got error: %v", err)
	}
}

func TestValidateResetState_FailedState(t *testing.T) {
	rel := release.NewReleaseRunForTest("test-fail", "main", "/test/repo")
	rel.ReconstructState(domainrelease.RunSnapshot{
		ID:       "test-fail",
		State:    domainrelease.StateFailed,
	...
	err := validateResetState(rel)
	if err != nil {
		t.Errorf("validateResetState() should allow failed state, got error: %v", err)
	}
}
