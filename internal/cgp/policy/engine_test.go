...
func TestEngine_Evaluate_RequireApprovalInt64Count(t *testing.T) {
	policy := NewPolicy("test-policy")
	policy.AddRule(*NewRule("int64-count", "Require review with int64 count").
		WithPriority(100).
		AddCondition("actor.kind", OperatorEqual, "agent").
		AddAction(ActionRequireApproval, map[string]any{"count": int64(2)}))
	...
	if result.Decision != cgp.DecisionApprovalRequired {
		t.Fatalf("Evaluate() Decision = %v, want %v", result.Decision, cgp.DecisionApprovalRequired)
	}
	if result.RequiredApprovers != 2 {
		t.Fatalf("Evaluate() RequiredApprovers = %d, want 2", result.RequiredApprovers)
	}
}
...
func TestEngine_Evaluate_LegacyAliases(t *testing.T) {
	p := NewPolicy("alias-policy")
	p.AddRule(*NewRule("legacy-risk", "Legacy risk_score alias works").
		WithPriority(100).
		AddCondition("risk_score", OperatorGreaterOrEqual, 0.8).
		AddAction(ActionBlock, map[string]any{"reason": "legacy risk alias matched"}))
	p.AddRule(*NewRule("legacy-bump", "Legacy bump_type alias works").
		WithPriority(90).
		AddCondition("bump_type", OperatorEqual, "major").
		AddAction(ActionRequireApproval, map[string]any{"count": float64(2)}))

	engine := NewEngine([]Policy{*p}, nil)
	proposal := cgp.NewProposal(
		cgp.NewHumanActor("john@example.com", "John"),
		cgp.ProposalScope{Repository: "owner/repo", CommitRange: "abc..def"},
		cgp.ProposalIntent{Summary: "Major release", Confidence: 0.9, SuggestedBump: cgp.BumpTypeMajor},
	)
	...
	if !result.Blocked {
		t.Fatalf("expected result to be blocked")
	}
	if result.BlockReason == "" {
		t.Fatalf("expected block reason to be set")
	}
...
func TestEngine_Evaluate_RuleTrace(t *testing.T) {
	p := NewPolicy("trace-policy")
	p.AddRule(*NewRule("high-risk", "Match high risk").
		WithPriority(100).
		AddCondition("risk.score", OperatorGreaterOrEqual, 0.8).
		AddAction(ActionRequireApproval, map[string]any{"count": 1}))
	p.AddRule(*NewRule("agent-only", "Match agent actor kind").
		WithPriority(50).
		AddCondition("actor.kind", OperatorEqual, "agent").
		AddAction(ActionAddRationale, map[string]any{"message": "agent path"}))
	...
	if len(result.RuleTrace) != 2 {
		t.Fatalf("expected 2 rule traces, got %d", len(result.RuleTrace))
	}
	if !result.RuleTrace[0].Matched {
		t.Fatalf("expected first rule to match")
	}
	if len(result.RuleTrace[0].Conditions) != 1 || !result.RuleTrace[0].Conditions[0].Matched {
		t.Fatalf("expected first rule condition to match")
	}
	if result.RuleTrace[1].Matched {
		t.Fatalf("expected second rule to not match")
	}
	if len(result.RuleTrace[1].Conditions) != 1 || result.RuleTrace[1].Conditions[0].Matched {
		t.Fatalf("expected second rule condition to not match")
	}
}
