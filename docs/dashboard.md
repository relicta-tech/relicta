...
Configure the dashboard in your `.relicta.yaml`:
...
Default behavior:
- The default config uses `none` for local development convenience.
- For shared environments, explicitly set `mode: api_key`.
...
3. Verify the `.relicta.yaml` exists
